...
<script>
    function showDefault() {
        ...
        $("#resultname").html('<h1 id="blinkText" style="color:#2A5969;">Check bib here</h1>');
        $("#contest").html("");
        $("#resultBib").html("");
        $("#resultTime").html("");
        $("#pace").html("");
            ...
            success: function(data) {
                ...
                if (data.status == 200) {
                    ...
                    if (!data.data.time || data.data.time.trim() === "" || data.data.time === "00:00:00") {
                        console.log("⚠️ Time kosong atau 00:00:00 → sembunyikan box result dan congrats");
                        $("#resultSection").hide();
                        $("#congrats").hide();
                    } else {
                        $("#resultSection").show();
...
<style>
    ...
    body {
        ...
        background-image: url('/img/Jeparundise.jpg');
...
<div class="bibTag">
    <div class="d-flex flex-column justify-content-center align-items-center" style="min-height: 85vh;">
        <div class="text-center pt-2">
            <img src="/img/logo_Jeparundise.png" alt="SSR25 logo" class="img-logo" style="margin-bottom: 10px;">
            <h4 id="congrats" class="text-uppercase py-2 px-md-5 mb-2"
                style="background-color: #24ADAC; color: #FFFFFF;">CONGRATULATION</h4>
            <h2 class="text-uppercase" id="resultname" style="color: #3A3A3A;">NAMA</h2>
            <div class="d-flex justify-content-center align-items-center" style="gap: 60px; margin-top: 10px;">
                <div>
                    <h5 class="text-uppercase text-center" id="contest" style="color: #3A3A3A"></h5>
                    <h3 class="text-uppercase" id="resultBib" style="color: #3A3A3A;"></h3>
                </div>
                <div id="resultSection" style="border: 4px solid #3A3A3A; border-radius: 15px; padding: 20px; margin-top: 10px;">
                    <h4 id="resultTime" style="color: #3A3A3A">RESULT : TIME</h4>
                </div>
            </div>
        </div>
    </div>
